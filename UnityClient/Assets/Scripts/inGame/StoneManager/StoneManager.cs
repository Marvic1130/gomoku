...
public class StoneManager : MonoBehaviour
{
    ...
    // CheckRule()을 통해 33금수가 검출이 되었는지 & 첫수가 두어져있는지를 알려주는 변수
                    ...
                    else if (hit.collider.gameObject.tag != "Stone")
                    {
                        ...
                        if(m_isFirstMove)
                        {
                            ...
                            if (m_ruleManager.CheckWin(player, m_currentBoardStateInit.m_Row, m_currentBoardStateInit.m_Col))
                            {
                                ...
                                UIobj.GetComponent<RectTransform>().sizeDelta = new Vector3(3200, 6000);
                                m_UI.transform.Find("Show Board").gameObject.SetActive(true);
                                ...
                                if (player == 1)
                                    ...
                                    data.Data("Lose");
                                jsonData = data.ObjectToJson(data);
                                Debug.Log(jsonData);
                                StartCoroutine(data.SendJsonData());
